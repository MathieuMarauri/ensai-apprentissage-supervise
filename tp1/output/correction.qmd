...
Le modèle des $K$ plus-proches-voisins (_K-nearest-neighbours_ en anglais) est un modèle d'apprentissage supervisé utilisable en classification et régression. Soient $(\textbf{x}_i, y_i)$ pour $i = 1, \dots, n$ des données d'apprentissage et $\textbf{x}$ le point à prédire. Pour effectuer la prédiction de la valeur $y$ de $\textbf{x}$, on calcule la distance qui sépare $\textbf{x}$ de chaque $\textbf{x}_i$. On sélectionne ensuite les $k$ points $(\textbf{x}_1, \dots, \textbf{x}_k)$ les plus proches de $\textbf{x}$ puis on détermine la classe à prédire en faisant un vote à la majorité sur $(y_1, \dots, y_k)$ pour la classification et en prenant la moyenne $\frac{1}{k} \sum_{j=1}^{k}y_j$ pour la regression.


<div style="text-align:center; padding: 20px 10px;">
![Modèle des plus proches voisins expliqué visuellement.](images/knn_explique.png){width=70%}
</div>
...
<div style="text-align: center; padding: 20px 10px;">
![Le fléau de la dimension. Plus le nombre de dimensions augmente, plus le nombre de points dans la sphére unité va diminuer.](images/curse-dimensionality.png){width=70%}
</div>

### La règle de décision

Dans le cas d'une classification, la règle de décision associée aux $k$ plus-proches-voisins est le vote à la majorité. Exemple : pour $k=5$, si 3 voisins ont une valeur $y$ de $A$ et 2 voisins ont une valeur $y$ de $B$ alors la prédiction sera $\widehat{y}=A$.
Dans le cadre de la régression c'est la moyenne des valeurs de $Y$ des 5 voisins sélectionnés qui sera utilisée.
...
Il est important de noter que le modèle est construit sur les données d'apprentissage et que l'erreur est calculée à partir de prédictions effectuées sur des données différentes, les données de test.
...
#### Importance des échantillons d'apprentissage, validation et test

Le calcul de l'erreur de généralisation d'un modèle doit se faire sur des données qui n'ont jamais servi dans la construction du modèle. 
