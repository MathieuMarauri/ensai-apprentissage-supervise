...
* Parmi les femmes, 20% prennent du sucre dans leur café
* Parmi les hommes, 10% prennent du sucre dans leur café
...
Le modèle des $K$ plus-proches-voisins (_K-nearest-neighbours_ en anglais) est un modèle d'apprentissage supervisé utilisable en classification et régression. Soient $(\textbf{x}_i, y_i)$ pour $i = 1, \dots, n$ des données d'apprentissage et $x$ le point à prédire. Pour effectuer la prédiction de la valeur $y$ de $\textbf{x}$, on calcule la distance qui sépare $x$ de chaque $x_i$. On sélectionne ensuite les $k$ points $(\textbf{x}_1, \dots, \textbf{x}_k)$ les plus proches de $\textbf{x}$ puis on détermine la classe à prédire en faisant un vote à la majorité sur $(y_1, \dots, y_k)$ pour la classification et en prenant la moyenne $\frac{1}{k} \sum_{j=1}^{k}y_j$ pour la regression.
...
![Le fléau de la dimension. Plus le nombre de dimension, plus le nombre de points dans la sphére unité va diminuer.](images/curse-dimensionality.png){width=70%}
</div>
